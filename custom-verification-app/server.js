...
async function createOrFindCustomerAccount(email) {
  ...
  const customerSearchResponse = await fetch(`https://gh5rsb-rj.myshopify.com/admin/api/2024-10/customers/search.json?query=email:${encodeURIComponent(email)}`, {
    method: 'GET',
    headers: {
  ...
  if (customerSearchResult.customers.length > 0) {
    ...
    const response = await fetch(`https://gh5rsb-rj.myshopify.com/admin/api/2024-10/customers.json`, {
      method: 'POST',
      headers: {
        'X-Shopify-Access-Token': adminAccessToken,
        'Content-Type': 'application/json',
...
async function sendAccountActivationEmail(email) {
  ...
  const sendInviteResponse = await fetch(`https://gh5rsb-rj.myshopify.com/admin/api/2024-10/customers/${customerId}/send_invite.json`, {
    method: 'POST',
    headers: {
      'X-Shopify-Access-Token': adminAccessToken,
      'Content-Type': 'application/json',
